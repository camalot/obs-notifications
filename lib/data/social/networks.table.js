...
let all = () => {
	return new Promise((resolve, reject) => {
		...
		let data = [];
		_database.sql.each(`SELECT rowid AS id, created, name, image, color, background, background2, system FROM ${_name} ORDER BY name`, {}, (err, row) => {
				if (err) {
					return reject(err);
				}
				data.push({
...
let add = (data) => {
	return new Promise((resolve, reject) => {
		...
		let utc = _database.utcNow();
		_database.sql.run(`INSERT INTO ${_name} (created, name, image, color, background, background2, system) VALUES ($created, $name, $image, $color, $background, $background2, 0)`,
			{ $created: utc, $name: data.name, $image: data.image, $color: data.color, $background: data.background, $background2: data.background2 }, (err) => {
				if (err) {
					return reject(err);
				}
				return resolve({
