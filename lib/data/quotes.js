...
let open = () => {
	return new Promise((resolve, reject) => {
		try {
			_database = new sqlite3.Database('data/quotes.sqlite', sqlite3.OPEN_CREATE | sqlite3.OPEN_READWRITE);
			_connected = true;
			return resolve(this);
		} catch (err) {
			return reject(err);
...
let get = (id) => {
	return new Promise((resolve, reject) => {
		...
		let data = null;
		_database.each("SELECT rowid AS id, text, created, user FROM quotes where rowid = $id LIMIT 1", { $id: id }, (err, row) => {
			if(err) {
				return reject(err);
			}
...
let all = (category) => {
	return new Promise((resolve, reject) => {
		...
		let data = [];
		_database.each("SELECT rowid AS id, text, created, user FROM quotes where category = $category", { $category: category }, (err, row) => {
			if (err) {
				return reject(err);
			}
...
let random = (category) => {
	return new Promise((resolve, reject) => {
		...
		_database.each("SELECT rowid as id, text, created, user FROM quotes where category = $category ORDER BY RANDOM() LIMIT 1", { $category: category }, (err, row) => {
			if (data === null) {
				data = {
					id: row.id,
					created: row.created,
