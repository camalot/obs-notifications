...
let all = (category) => {
	return new Promise((resolve, reject) => {
		...
		let data = [];
		_database.each("SELECT rowid AS id, text, created, user FROM quotes where category = $category", { $category: cat }, (err, row) => {
			if (err) {
				return reject(err);
			}
...
let random = (category) => {
	return new Promise((resolve, reject) => {
		...
		_database.each("SELECT rowid as id, text, created, user FROM quotes where category = $category ORDER BY RANDOM() LIMIT 1", { $category: cat }, (err, row) => {
			if (data === null) {
				data = {
					id: row.id,
					created: row.created,
