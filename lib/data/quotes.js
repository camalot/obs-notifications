...
let open = () => {
	return new Promise((resolve, reject) => {
		try {
			_database = new sqlite3.Database(`${config.databasePath}/quotes.sqlite`, sqlite3.OPEN_CREATE | sqlite3.OPEN_READWRITE);
			_connected = true;
			return create()
...
let all = (category) => {
	return new Promise((resolve, reject) => {
		...
		let data = [];
		_database.each("SELECT rowid AS id, text, created, user, category FROM quotes where category = $category OR 1 = $all", { $category: cat || undefined, $all: all ? 1 : 0 }, (err, row) => {
			if (err) {
				return reject(err);
			}
...
let random = (category) => {
	return new Promise((resolve, reject) => {
		...
		_database.each("SELECT rowid as id, text, created, user, category FROM quotes where category = $category OR 1 = $all ORDER BY RANDOM() LIMIT 1", { $category: cat || undefined, $all: all ? 1 : 0 }, (err, row) => {
			if (data === null) {
				data = {
					id: row.id,
					created: row.created,
