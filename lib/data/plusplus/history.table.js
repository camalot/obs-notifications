...
let remove = (text) => {
	return new Promise((resolve, reject) => {
		...
		_database.sql.run(`DELETE FROM ${_name} where text = $text`, { $text: text }, (err) => {
			if (err) {
				return reject(err);
			}

			console.log(`deleted plusplus vote: ${text}`);
			return resolve();
		});
	});
};
...
let truncate = (text) => {
	return new Promise((resolve, reject) => {
		...
		_database.sql.run(`DELETE FROM ${_name}`, { $text: text }, (err) => {
			if (err) {
				return reject(err);
			}

			console.log(`truncate table ${_name}`);
			return resolve();
		});
	});
};
